		...
		bool _use_ccpem_pipeliner, bool _do_projdir, FileName _switch_to):Fl_Window(w,h,title)
{
	...
	else
	{
		...
		browse_grp[nr_browse_tabs] = new Fl_Group(WCOL0, 2, 550, 615-MENUHEIGHT);
		browser->add("2D consensus");
		gui_jobwindows[nr_browse_tabs] = new JobWindow();
		gui_jobwindows[nr_browse_tabs]->initialise(PROC_CLASS2D_CONSENSUS);
		browse_grp[nr_browse_tabs]->end();
		nr_browse_tabs++;
...
void GuiMainWindow::cb_print_cl_i()
{
	...
	bool preview_is_continue = is_main_continue && !do_overwrite_continue;
	bool preview_is_overwrite = is_main_continue && do_overwrite_continue;
...
{
	if (current_job < 0 || current_job >= pipeline.processList.size())
		return;
