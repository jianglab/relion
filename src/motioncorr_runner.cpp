...
void MotioncorrRunner::initialise()
{
	...
	pre_exposure_micrographs.clear();
		...
		FOR_ALL_OBJECTS_IN_METADATA_TABLE(MDin)
		{
			...
			if (MDin.containsLabel(EMDL_MICROGRAPH_DOSE_RATE)){
				double curr_dose_mic;
				MDin.getValue(EMDL_MICROGRAPH_DOSE_RATE, curr_dose_mic);
				dose_per_frame_vec.push_back(curr_dose_mic);
			}
...
bool MotioncorrRunner::executeMotioncor2(Micrograph &mic, int rank)
{
	...
	if (do_dose_weighting)
	{
		...
		double dose_per_frame_mic = mic.getDosePerFrame();
		
		if (dose_per_frame_mic == -1)
		{
			command += " -FmDose " + floatToString(dose_per_frame);
		}
		else
		{
			command += " -FmDose " + floatToString(dose_per_frame_mic);
		}
...
skip_fitting:
	...
	if (do_dose_weighting) {
		...
		double dose_per_frame_mic = mic.getDosePerFrame();
		double dose_per_frame_to_use = dose_per_frame;
		logfile << "Dose read from mic: " << dose_per_frame_mic << std::endl;
		if (dose_per_frame_mic != -1) {
			dose_per_frame_to_use = dose_per_frame_mic;
		}
		 
		std::vector <RFLOAT> doses(n_frames);
		for (int iframe = 0; iframe < n_frames; iframe++) {
			// dose AFTER each frame.
			doses[iframe] = mic.pre_exposure + dose_per_frame_to_use * (frames[iframe] + 1);
		...
		logfile << " done" << std::endl;
		logfile << "Used dose: " << dose_per_frame_to_use <<std::endl;
		RCTOC(TIMING_REAL_SPACE_INTERPOLATION);
